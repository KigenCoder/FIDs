...
class AppAuthController extends Controller
{
    ...
    public function uploadMarketData(Request $request)
    {
        ...
        foreach ($marketData as $indicator_id => $price) {
            ...
            if (strcasecmp($indicator_id, $civilSecurityID) == 0) {
                //CIVIL INSECURITY - Monthly data
                $existingRecord = MarketData::where("year_name", $year_name)
                    ->where("month_id", $month_id)
                    ->where("market_id", $market_id)
                    ->where("indicator_id", $civilSecurityID)
                    ->first();
            } else {
                //NON CIVIL INSECURITY - weekly data
                $existingRecord = MarketData::where("year_name", $year_name)
                    ->where("month_id", $month_id)
                    ->where("week", $week)
                    ->where("market_id", $market_id)
    ...
    protected function civilSecurity()
    {
